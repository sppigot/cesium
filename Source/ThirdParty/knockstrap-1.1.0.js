...
define(function() {
    ...
    var createUnwrapProperties = function (ko) {
    ...
    var tooltip = function(ko) {
		...
		function attachToKo(koAttach) {
			var ko = koAttach;
			initTemplates(ko);
			ko.utils.uniqueId = uniqueId;
    	ko.utils.unwrapProperties = createUnwrapProperties(ko);
    	ko.bindingHandlers.alert = alert(ko);
    	ko.virtualElements.allowedBindings.alert = true;
    	ko.bindingHandlers.carousel = carousel(ko);
    	ko.bindingHandlers.checkbox = checkbox(ko);
    	ko.bindingHandlers.modal = modal(ko);
    	ko.bindingHandlers.popover = popover(ko);
    	ko.bindingHandlers.progress = progress(ko);
    	ko.bindingHandlers.radio = radio(ko);
    	ko.bindingHandlers.toggle = toggle(ko);
    	ko.bindingHandlers.tooltip = tooltip(ko);
		}
